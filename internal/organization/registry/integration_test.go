...
func TestCanonicalRegistryAgainstPostgreSQL(t *testing.T) {
	...
	// 43 imported + 5 proposed = the 48 roles asserted above. These counts
	...
	if imported != 43 || proposed != 5 {
		t.Fatalf("imported=%d proposed=%d (expected 43/5 per docs/canonical/role-catalog.yaml)", imported, proposed)
	}
