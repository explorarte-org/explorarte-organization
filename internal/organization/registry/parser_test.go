...
func TestLoadCurrentCanonicalDocuments(t *testing.T) {
	...
	if snapshot.Counts.Roles != 47 || snapshot.Counts.ImportedRoles != 45 || snapshot.Counts.ProposedRoles != 2 {
	...
	egress, err := modelegress.LoadCanonicalPolicy(canonicalDirForTest(t), modelegress.ProductiveLoadOptions([]string{"deepseek", "openai_compatible", "openai_responses", "gemini"}))
	if err != nil {
		t.Fatal(err)
	}
