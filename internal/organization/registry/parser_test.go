...
func TestLoadCurrentCanonicalDocuments(t *testing.T) {
	...
	if snapshot.Counts.Roles != 48 || snapshot.Counts.ImportedRoles != 43 || snapshot.Counts.ProposedRoles != 5 {
	...
	egress, err := modelegress.LoadCanonicalPolicy(canonicalDirForTest(t), modelegress.ProductiveLoadOptions([]string{"deepseek", "openai_compatible", "openai_responses", "gemini", "xai"}))
	if err != nil {
		t.Fatal(err)
	}
