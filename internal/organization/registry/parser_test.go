...
func TestLoadCurrentCanonicalDocuments(t *testing.T) {
	...
	egress, err := modelegress.LoadCanonicalPolicy(canonicalDirForTest(t), modelegress.ProductiveLoadOptions([]string{"deepseek", "openai_compatible", "openai_responses", "gemini", "mimo", "xai"}))
	if err != nil {
		t.Fatal(err)
	}
