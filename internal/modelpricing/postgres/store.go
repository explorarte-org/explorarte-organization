...
func (s *Store) ListTiers(ctx context.Context, providerID, providerModelID string, asOf time.Time) ([]modelpricing.PriceTier, error) {
...
FROM model_pricing
WHERE provider_id=$1 AND provider_model_id=$2 AND effective_at <= $3`,
		providerID, providerModelID, asOf.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&tier.EffectiveAt, &tier.CreatedAt,
		); err != nil {
			return nil, err
		}
...
ON CONFLICT (provider_id, provider_model_id, context_tier_name, effective_at) DO NOTHING
RETURNING created_at`,
		tier.ProviderID, tier.ProviderModelID, tier.ContextTierName, tier.MinInputTokens,
		int64(tier.InputPriceNanosPerMillion), cachedInputNanos, cacheWriteNanos, int64(tier.OutputPriceNanosPerMillion), tier.EffectiveAt.UTC())
