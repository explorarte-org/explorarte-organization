...
// WithNoRetries is an explicit operator-only mode for a bounded one-shot
// probe. It changes task attempt ceilings, not provider semantics or success
// validation: a failed attempt remains failed/blocked and is never silently
// driven again by this orchestrator instance.
func WithNoRetries() OrchestratorOption {
	return func(o *Orchestrator) { o.noRetries = true }
}

func (o *Orchestrator) maxAttempts(defaultAttempts int) int {
	if o.noRetries {
		return 1
	}
...
func (o *Orchestrator) createCEOPlanTask(ctx context.Context, root TaskRecord) (TaskRecord, bool, error) {
	...
	task, reused, err := o.coordinatedChildren().Materialize(ctx, childRequest{Root: root, Sender: root, Depth: DepthCEOPlan, Command: CreateTaskCommand{
		...
		Priority: 100, MaxAttempts: o.maxAttempts(3),
...
func (o *Orchestrator) createLeaderPlanTask(ctx context.Context, root TaskRecord, req DepartmentRequest, leader RoleRef) (TaskRecord, bool, error) {
	...
	task, reused, err := o.coordinatedChildren().Materialize(ctx, childRequest{Root: root, Sender: root, Depth: DepthDepartmentPlan, Command: CreateTaskCommand{
		...
		Priority:           req.Priority, MaxAttempts: o.maxAttempts(3), CorrelationID: root.CorrelationID, CausationID: taskCausation(root.ID),
...
func (o *Orchestrator) driveTypedTask(ctx context.Context, root TaskRecord, task TaskRecord, schema json.RawMessage, purpose ExecutionPurpose, validate func(InvocationResult) error) (TaskRecord, error) {
	...
	if purpose == PurposeDepartmentWorker {
		...
		validate = func(result InvocationResult) error {
			...
			if err := verifyDeliverableFitsTheReview(root, result); err != nil {
				return err
			}
			if len(required) > 0 {
