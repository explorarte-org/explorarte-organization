...
func providerRequestFixture() CanonicalRequest {
	...
	return CanonicalRequest{
		...
		ModelInput:           PreparedModelInput{Envelope: ModelInputEnvelope{SchemaVersion: ModelInputEnvelopeSchemaV1, ContextSnapshotID: 17, StablePrefixDigest: SHA256Bytes([]byte("prefix")), InputClassificationsHash: SHA256Bytes([]byte("classes"))}, CanonicalDigest: SHA256Bytes([]byte("model-input"))},
...
func TestBuildProviderRequestEvidenceIsDeterministicAndContentIndependent(t *testing.T) {
	...
	fourthRequest.ModelInput.CanonicalDigest = SHA256Bytes([]byte("different model input"))
	fourth, err := BuildProviderRequestEvidence(fourthRequest, descriptor)
	if err != nil {
		t.Fatal(err)
	}
	if fourth.RequestHash == first.RequestHash {
		t.Fatal("request evidence did not bind model input digest")
	}
}
