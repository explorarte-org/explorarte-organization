...
const invocationColumns = `id,organization_id,organization_revision_id,task_id,attempt_id,dispatch_actor_role_id,subject_role_id,context_snapshot_id,purpose,model_profile_id,model_profile_version_id,provider_id,provider_model_id,required_capabilities,output_mode,output_schema,max_output_tokens,temperature,thinking_mode,idempotency_key,request_hash,status,error_code,cancel_requested_at,deadline,correlation_id,causation_id,created_at,updated_at,terminal_at`
...
func scanInvocation(row scanner) (modelruntime.Invocation, error) {
	...
	var errorCode, correlationID, causationID *string
	err := row.Scan(&v.ID, &v.OrganizationID, &v.OrganizationRevisionID, &v.TaskID, &v.AttemptID, &v.DispatchActorRoleID, &v.SubjectRoleID, &v.ContextSnapshotID, &v.Purpose, &v.ModelProfileID, &v.ModelProfileVersionID, &v.ProviderID, &v.ProviderModelID, &caps, &v.OutputMode, &schema, &v.MaxOutputTokens, &v.Temperature, &v.ThinkingMode, &v.IdempotencyKey, &v.RequestHash, &v.Status, &errorCode, &v.CancelRequestedAt, &v.Deadline, &correlationID, &causationID, &v.CreatedAt, &v.UpdatedAt, &v.TerminalAt)
	if err != nil {
