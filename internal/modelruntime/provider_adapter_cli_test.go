...
func TestProviderOutcomeCLIRequestNotSentCannotCarryProcessExit(t *testing.T) {
	zero := 0
	outcome := ProviderOutcome{
		...
		ProcessExitCode:       &zero,
		ErrorClass:            "process",
		ErrorCode:             "process_not_started",
		ResponseSchemaVersion: "claude.code.print.response.v1",
	}
	if err := outcome.Validate(); err == nil {
		t.Fatal("request_not_sent accepted process exit evidence")
	}
}
