...
func validRequest(deadline time.Time) modelruntime.CanonicalRequest {
	return modelruntime.CanonicalRequest{
		...
		ProviderID: ProviderID, ProviderModelID: "gemini-3.6-flash",
...
func TestConfigRequiresSecureExplicitEndpointAndCredentialReference(t *testing.T) {
	...
	for name, endpoint := range map[string]string{
		"plain HTTP":      "http://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
		"userinfo":        "https://user@generativelanguage.googleapis.com/v1beta/openai/chat/completions",
		"query":           "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions?x=1",
		"fragment":        "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions#x",
		"v1 prefix wrong": "https://generativelanguage.googleapis.com/v1/chat/completions",
		"wrong path":      "https://generativelanguage.googleapis.com/v1beta/openai/responses",
	} {
	...
	cfg := adapterConfig("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", "relative-token")
	if err := cfg.Validate(); err == nil {
		t.Fatal("relative credential path unexpectedly accepted")
	}
	if err := adapterConfig("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", credential).Validate(); err != nil {
		t.Fatal(err)
	}
}
...
func TestDispatchSendsBoundedCanonicalRequestAndNormalizesResponse(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if payload["model"] != "gemini-3.6-flash" || payload["stream"] != false {
	...
	cfg := adapterConfig(server.URL+"/v1beta/openai/chat/completions", credential)
...
func TestDispatchUsesStructuredOutputWithoutPersistingSchemaContent(t *testing.T) {
	...
	defer server.Close()
	adapter, err := newAdapter(adapterConfig(server.URL+"/v1beta/openai/chat/completions", credential), server.Client(), time.Now)
	if err != nil {
		t.Fatal(err)
	}
	request := validRequest(time.Now().Add(time.Minute))
...
func TestTransportFailureIsAmbiguousAndCircuitOpens(t *testing.T) {
	...
	adapter, err := newAdapter(adapterConfig("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", credential), client, func() time.Time { return now })
	if err != nil {
		t.Fatal(err)
	}
