...
// surface. Mirrors internal/modelruntime/adapter/deepseek's failureTelemetry;
// cacheHitTokens/cacheMissTokens map xAI's prompt_tokens_details.cached_tokens
// the same way promptCacheSplit already does for RawResponse.
...
// row so a failure row is self-sufficient without a join. outputTokens uses
// billedOutputTokens() -- reasoning tokens included -- the same figure the
// success path already bills, so a truncated-empty or content-filtered
// failure after a reasoning burn is not undercounted here either.
func (t failureTelemetry) withUsage(usage chatUsage) failureTelemetry {
	...
	if hit, miss, reported := promptCacheSplit(usage); reported {
		t.cacheHitTokens = &hit
		t.cacheMissTokens = &miss
	}
	return t
}
...
func (a *Adapter) Dispatch(ctx context.Context, request modelruntime.CanonicalRequest) (modelruntime.RawResponse, error) {
	...
	if readErr != nil {
		...
		outcome := responseErrorOutcome(response.StatusCode, providerRequestID, responseHash, "response", StreamErrorCode(readErr, "response_read_failed"), retryable, telemetry)
