...
func New(evaluator Evaluator, revisions RevisionReader, organizationID string) (*Gate, error) {
	if evaluator == nil {
		return nil, errors.New("rag authorization gate requires evaluator")
	}
	if revisions == nil {
		return nil, errors.New("rag authorization gate requires revision reader")
	}
	organizationID = strings.TrimSpace(organizationID)
	if organizationID == "" {
		return nil, errors.New("rag authorization gate requires organization ID")
	}
	return &Gate{evaluator: evaluator, revisions: revisions, organizationID: organizationID}, nil
}

var _ rag.AuthorizationGate = (*Gate)(nil)

func (g *Gate) Authorize(ctx context.Context, request rag.AuthorizationRequest) error {
	if request.OrganizationID != g.organizationID {
		return fmt.Errorf("%w: rag organization mismatch", authorization.ErrCapabilityDenied)
	}
	revision, err := g.revisions.GetCurrentRevision(ctx, g.organizationID)
	if err != nil {
		return fmt.Errorf("resolve rag authorization revision: %w", err)
	}
