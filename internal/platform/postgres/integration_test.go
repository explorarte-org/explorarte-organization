...
func TestPostgresMigrationsAndUnitOfWork(t *testing.T) {
	...
	if len(loaded) != 15 {
		t.Fatalf("loaded migrations = %d, want 15", len(loaded))
	}
	if err := store.UnitOfWork().WithinTransaction(ctx, pgx.TxOptions{}, func(ctx context.Context, tx pgx.Tx) error {
	...
	for _, table := range []string{
		"organizational_memory_versions",
		"organizational_memory_entries",
		"organizational_memory_evidence_refs",
		"organizational_memory_state_events",
	...
	if err != nil {
		t.Fatalf("restore migration 000015: %v", err)
	}
	if len(restored.Applied) != 1 || restored.Current != 15 {
		t.Fatalf("restore migration result = %+v, want only 000015", restored)
	}
}
