...
func renderConversationContract(prior []Message, newMessage Message) string {
	...
	b.WriteString("- You cannot approve a campaign; only the owner can. When the owner wants to approve a recommended proposal, call campaign.prepare_owner_approval and give them the exact command it returns. Never tell the owner an approval was made unless they report they ran it.\n\n")
