...
// TestOpenBGEM3SemanticSearchRequiresReadySidecarAtStartup is R30.1-6:
// mirrors internal/rag/bootstrap's equivalent test exactly — the bootstrap
// of the BGE-M3 profile must call Adapter.Healthy before ever returning
// usable SemanticSearchDeps.
func TestOpenBGEM3SemanticSearchRequiresReadySidecarAtStartup(t *testing.T) {
	...
	deps, err := openBGEM3SemanticSearch(nil, nil)
	if err == nil {
...
func TestOpenBGEM3SemanticSearchFailsWhenSidecarUnreachable(t *testing.T) {
	...
	if _, err := openBGEM3SemanticSearch(nil, nil); err == nil {
