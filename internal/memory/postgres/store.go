...
WHERE organization_id=$1 AND entry_key=$2`, s.organizationID, entry.SupersedesEntryID).Scan(&supersededRole); err != nil {
		...
		if supersededRole != entry.RoleID {
...
INSERT INTO organizational_memory_versions (
    ...
    source_run_id, canonical_hash, proposed_by_role_id,
...
ON CONFLICT (organization_id, canonical_hash) DO NOTHING`,
		...
		entry.SourceRunID, canonicalHash, entry.ProposedBy,
...
SELECT v.entry_key, v.organization_id, v.role_id, v.category, v.problem, v.correction,
       v.source_run_id, e.status, v.proposed_by_role_id, e.reviewer_role_id,
...
WHERE v.organization_id=$1 AND v.entry_key=$2`, organizationID, entryID).Scan(
		...
		&entry.SourceRunID, &status, &entry.ProposedBy, &reviewer,
