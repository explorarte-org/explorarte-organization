...
func sourceRecord(entry memory.Entry) (contextengine.SourceRecord, error) {
	...
	payload, err := json.Marshal(renderedMemory{
		SchemaVersion: "approved-memory.context.v1",
		Category:      entry.Category,
		Problem:       entry.Problem,
		Correction:    entry.Correction,
		SourceRunID:   entry.SourceRunID,
		EvidenceRefs:  append([]memory.EvidenceRef(nil), entry.EvidenceRefs...),
	})
	if err != nil {
	...
	record := contextengine.SourceRecord{
		Kind:                 contextengine.SourceApprovedMemory,
		Reference:            entry.ID,
		Version:              "organizational-memory.v1:" + canonicalHash,
		AuthorityTier:        contextengine.TierApprovedMemory,
