...
func TestProposeReviewDeprecateArchive(t *testing.T) {
	...
	if entry.Status != StatusCandidate || entry.Revision != 1 { t.Fatalf("candidate state=%s revision=%d", entry.Status, entry.Revision) }
	if entry.EvidenceRefs[0].Reference != "task:42:evidence:1" { t.Fatalf("evidence references not deterministic: %+v", entry.EvidenceRefs) }
	clock.now = now.Add(time.Minute)
	entry, err = service.Review(entry, Review{Outcome: ReviewApprove, ReviewerID: "empresa/human"})
	if err != nil { t.Fatal(err) }
	if entry.Status != StatusApproved || entry.ReviewerID != "empresa/human" || entry.ReviewedAt == nil || entry.Revision != 2 { t.Fatalf("approved entry=%+v", entry) }
	...
	if err != nil || entry.Status != StatusArchived || entry.Revision != 4 { t.Fatalf("archived entry=%+v err=%v", entry, err) }
}
...
func TestSourceKindIsRequiredAndSimulationIsPreserved(t *testing.T) {
	...
	command.SourceKind = ""
	if _, err := service.Propose(command); !errors.Is(err, ErrInvalidEntry) { t.Fatalf("missing source kind error=%v", err) }
	command.SourceKind = SourceSimulation
	entry, err := service.Propose(command)
	if err != nil { t.Fatal(err) }
	if entry.SourceKind != SourceSimulation { t.Fatalf("source kind=%s", entry.SourceKind) }
}
...
func TestClinicalAndSecretContentCannotBecomeCandidate(t *testing.T) {
	...
	for _, class := range []DataClass{DataClinical, DataSecret} {
		...
		if !errors.Is(err, ErrForbiddenDataClass) { t.Fatalf("class %s error=%v", class, err) }
	}
}
...
func TestSanitizedAdmissionRequiresSanitizationEvidence(t *testing.T) {
	...
	command := validCommand(now)
	command.Admission.SanitizationEvidenceRef = ""
	if _, err := service.Propose(command); !errors.Is(err, ErrInvalidAdmission) { t.Fatalf("error=%v", err) }
}
...
func TestOrganizationalAdmissionCannotCarrySanitizationEvidence(t *testing.T) {
	...
	if _, err := service.Propose(command); !errors.Is(err, ErrInvalidAdmission) { t.Fatalf("error=%v", err) }
	command.Admission.SanitizationEvidenceRef = ""
	if _, err := service.Propose(command); err != nil { t.Fatal(err) }
}
...
func TestAdmissionMustPrecedeCandidate(t *testing.T) {
	...
	if _, err := service.Propose(command); !errors.Is(err, ErrInvalidEntry) { t.Fatalf("error=%v", err) }
}

func TestApprovedEntryRequiresReviewer(t *testing.T) {
	now := time.Date(2026, 8, 7, 4, 0, 0, 0, time.UTC)
	service := NewService(&fixedClock{now: now})
	entry, err := service.Propose(validCommand(now)); if err != nil { t.Fatal(err) }
	entry.Status = StatusApproved
	if err := entry.Validate(); err == nil { t.Fatal("approved entry without review provenance validated") }
}

func TestRejectedEntryCanOnlyArchive(t *testing.T) {
	now := time.Date(2026, 8, 7, 4, 0, 0, 0, time.UTC)
	clock := &fixedClock{now: now}; service := NewService(clock)
	entry, err := service.Propose(validCommand(now)); if err != nil { t.Fatal(err) }
	clock.now = now.Add(time.Minute)
	entry, err = service.Review(entry, Review{Outcome: ReviewReject, ReviewerID: "empresa/human"}); if err != nil { t.Fatal(err) }
	if _, err = service.Deprecate(entry); !errors.Is(err, ErrInvalidTransition) { t.Fatalf("error=%v", err) }
	clock.now = now.Add(2*time.Minute)
	entry, err = service.Archive(entry); if err != nil || entry.Status != StatusArchived { t.Fatalf("entry=%+v err=%v", entry, err) }
}
...
func TestTransitionMatrixIsDefaultDeny(t *testing.T) {
	...
	for _, from := range states { for _, to := range states { err := ValidateTransition(from,to); if allowed[[2]Status{from,to}] && err != nil { t.Fatalf("%s -> %s: %v",from,to,err) }; if !allowed[[2]Status{from,to}] && !errors.Is(err,ErrInvalidTransition) { t.Fatalf("%s -> %s error=%v",from,to,err) } } }
}

func TestCanonicalHashCommitsContentAdmissionAndSourceKind(t *testing.T) {
	now := time.Date(2026,8,7,4,0,0,0,time.UTC); clock := &fixedClock{now:now}; service := NewService(clock)
	entry, err := service.Propose(validCommand(now)); if err != nil { t.Fatal(err) }
	before, _ := entry.CanonicalHash()
	clock.now = now.Add(time.Minute)
	approved, err := service.Review(entry, Review{Outcome:ReviewApprove,ReviewerID:"empresa/human"}); if err != nil { t.Fatal(err) }
	after, _ := approved.CanonicalHash(); if before != after { t.Fatal("lifecycle changed canonical hash") }
	changed := approved; changed.Correction += " changed"; h,_ := changed.CanonicalHash(); if h == after { t.Fatal("content change did not change hash") }
	changed = approved; changed.Admission.EvidenceRef = "changed"; h,_ = changed.CanonicalHash(); if h == after { t.Fatal("admission change did not change hash") }
	changed = approved; changed.SourceKind = SourceSimulation; h,_ = changed.CanonicalHash(); if h == after { t.Fatal("source kind change did not change hash") }
}
