...
type memoryAdmissionInput struct {
	DataClass               memory.DataClass `json:"data_class"`
...
type memoryProposeInput struct {
	...
	EvidenceRefs      []memoryEvidenceInput  `json:"evidence_refs"`
	ProposedBy        string                 `json:"proposed_by"`
	Admission         memoryAdmissionInput   `json:"admission"`
...
type memoryReviewInput struct {
	...
	Outcome          memory.ReviewOutcome `json:"outcome"`
}

type memoryMutationInput struct {
	EntryID          string `json:"entry_id"`
	ExpectedRevision int64  `json:"expected_revision"`
	ActorRoleID      string `json:"actor_role_id"`
	Reason           string `json:"reason"`
...
func runMemory(args []string, stdout, stderr io.Writer) int {
	...
	defer cancel()
	store, runner, code := openDatabase(ctx, cfg, stderr, "memory")
	if code != exitOK {
		return code
	}
	defer store.Close()
	...
	runtime, err := memorybootstrap.Open(cfg, store)
	if err != nil {
	...
	case "propose":
		...
		jsonOutput, code := parseMemoryFile(args[1:], stderr, &input)
		if code != exitOK {
			return code
		}
		...
		refs := make([]memory.EvidenceRef, 0, len(input.EvidenceRefs))
		for _, ref := range input.EvidenceRefs {
			refs = append(refs, memory.EvidenceRef{Reference: ref.Reference, Digest: ref.Digest})
		}
		entry, reused, err := runtime.Manager.Propose(ctx, memory.ProposeRequest{
			Command: memory.ProposeCommand{
				ID: input.ID, OrganizationID: runtime.OrganizationID, RoleID: input.RoleID,
				Category: input.Category, Problem: input.Problem, Correction: input.Correction,
				SourceRunID: input.SourceRunID, EvidenceRefs: refs, ProposedBy: input.ProposedBy,
		...
		writeValue(stdout, jsonOutput, map[string]any{"entry": entry, "reused": reused})
		return exitOK
	case "review":
		...
		entry, err := runtime.Manager.Review(ctx, memory.ReviewRequest{
			Mutation: memory.MutationRequest{OrganizationID: runtime.OrganizationID, EntryID: input.EntryID, ExpectedRevision: input.ExpectedRevision, ActorRoleID: input.ActorRoleID, Reason: input.Reason},
			Outcome: input.Outcome,
		})
		if err != nil {
			return memoryCommandError(stderr, err)
		}
		writeValue(stdout, jsonOutput, entry)
		return exitOK
	case "deprecate", "archive":
		...
		request := memory.MutationRequest{OrganizationID: runtime.OrganizationID, EntryID: input.EntryID, ExpectedRevision: input.ExpectedRevision, ActorRoleID: input.ActorRoleID, Reason: input.Reason}
		var entry memory.Entry
		if args[0] == "deprecate" {
			entry, err = runtime.Manager.Deprecate(ctx, request)
		} else {
			entry, err = runtime.Manager.Archive(ctx, request)
		}
		if err != nil {
			return memoryCommandError(stderr, err)
		}
...
func parseMemoryFile(args []string, stderr io.Writer, target any) (bool, int) {
	flags := flag.NewFlagSet("memory file command", flag.ContinueOnError)
	flags.SetOutput(stderr)
	...
	if err != nil {
		fmt.Fprintf(stderr, "read memory input: %v\n", err)
		return false, exitUsage
	}
	if err := decodeMemoryStrict(raw, target); err != nil {
		fmt.Fprintf(stderr, "decode memory input: %v\n", err)
		return false, exitUsage
	}
	return *jsonOutput, exitOK
}

func decodeMemoryStrict(raw []byte, target any) error {
...
func memoryCommandError(stderr io.Writer, err error) int {
	fmt.Fprintf(stderr, "memory operation failed: %v\n", err)
	switch {
	...
	case errors.Is(err, memory.ErrRevisionConflict):
		return exitDrift
	case errors.Is(err, memory.ErrInvalidRequest), errors.Is(err, memory.ErrInvalidEntry),
		errors.Is(err, memory.ErrInvalidEvidenceRef), errors.Is(err, memory.ErrInvalidAdmission),
		errors.Is(err, memory.ErrForbiddenDataClass), errors.Is(err, memory.ErrInvalidTransition),
		errors.Is(err, memory.ErrInvalidReview), errors.Is(err, memory.ErrEntryNotFound),
		errors.Is(err, memory.ErrDuplicateCandidate), errors.Is(err, memory.ErrConflict):
		return exitInvalid
	default:
		return exitInternal
	}
