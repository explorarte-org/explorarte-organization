...
func runMemory(args []string, stdout, stderr io.Writer) int {
	...
	case "review":
		...
		entry, err := runtime.Manager.Review(ctx, memory.ReviewRequest{Mutation: memory.MutationRequest{OrganizationID: runtime.OrganizationID, EntryID: input.EntryID, ExpectedRevision: input.ExpectedRevision, ActorRoleID: input.ActorRoleID, Reason: input.Reason}, Outcome: input.Outcome})
		if err != nil {
			return memoryCommandError(stderr, err)
		}
