...
- worker to worker;
- worker directly to CEO;
- leader to another department's worker;
- execution principal role different from actor/sender role;
- disabled execution principal;
- cross-organization execution principal;
...
`Runtime.Observe` deterministically returns:
