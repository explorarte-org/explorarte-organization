...
- **L6.** `search_text`'s `data_class` inheritance (DESIGN.md §12B.1): a
  ...
  the snippet, never dropping it. (M2.4) Already closed at the M2.0
  layer — DESIGN.md's `context.search` OUTPUT line and
  `internal/contextdisclosure.SearchResult` both carry `data_class`; M2.4
