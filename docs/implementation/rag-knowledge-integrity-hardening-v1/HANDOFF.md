...
1. **`internal/testdbguard`** (paquete nuevo, 9 tests unitarios tras la fase 2 de cierre, 100% verde): `RequireTestDatabase(ctx, dsn, pool)` exige que el DSN nombre `explorarte_test` Y que `SELECT current_database()` en vivo confirme lo mismo — nunca confía en `ORG_ENVIRONMENT=test` porque el propio test lo auto-inyecta. `RequireDestructive` añade una segunda condición deliberada: `ORG_TEST_DESTRUCTIVE_DATABASE=explorarte_test` debe estar seteado explícitamente antes de permitir TRUNCATE/DownSQL.
