...
## Fase 7 — harness de evidencia web efímera

`internal/webevidence` es el harness completo (sin elegir proveedor real de búsqueda/fetch — `Fetcher` es una interfaz con solo un fake para tests): `Ingest` (fetch → `Sanitize` → hash → chunk → `Evidence` validada) → `RankChunks` (RRF léxico+vectorial, reutilizando exactamente el mismo principio de `internal/rag/postgres/hybrid_query.go`) → `NewCitation` (referencia acotada con URL/hash/fecha, nunca la página cruda). `Evidence.Validate()` exige TTL obligatorio y acotado (`MinTTL`=1 minuto, `MaxTTL`=7 días) — no existe ningún valor representable de "para siempre". Migración 000033 (`web_evidence`) replica esa obligatoriedad en la base: `expires_at NOT NULL CHECK (expires_at > captured_at)`, sin FK hacia ninguna tabla de RAG/Memory/embeddings, y toda lectura del store filtra `expires_at > now()` — una fila vencida es invisible aunque el reaper (`Store.Reap`, acotado por `limit`) todavía no la haya borrado físicamente.

"Nunca se convierte en instrucción del sistema" es estructural, no una política que se pueda olvidar aplicar: `Evidence` no tiene ningún campo que pueda representar `InstructionClass` distinto de "data" — la sanitización (`Sanitize`, con un catálogo real de patrones de inyección de prompt: "ignore previous instructions", "you are now", "system prompt", etc.) solo produce `SanitizationFinding`s de auditoría, nunca cambia cómo se clasifica el contenido. Probado con 5 muestras hostiles reales (incluyendo la exacta del fixture `r30-13-hostile-web-page`) que siguen validando como evidencia ordinaria pese a disparar hallazgos.

"Nunca se promueve automáticamente a RAG/Memory" también es estructural: `internal/webevidence` e `internal/webevidencefixtures` no importan `internal/rag` ni `internal/memory` — no existe ninguna llamada que este código pudiera hacer para promover evidencia, ni siquiera por error. Promover evidencia web a conocimiento organizacional permanente sigue exigiendo pasar por `rag.Manager.Propose`/`memory.Manager.Propose` con una `AdmissionAttestation` real, exactamente el mismo camino que cualquier otro candidato.

`internal/webevidencefixtures` (mismo patrón que `internal/decisiongraphfixtures`: `internal/evaluation/fixtures` se mantiene libre de dependencias) activa `r30-13-hostile-web-page` con un escenario real y lo corre a través del harness completo — ahora 4 de los 14 fixtures tienen runner real (antes 3). `cmd/orgctl/evaluation.go` corre ambos runners.

Probado contra Postgres real: guardar/leer/listar evidencia, invisibilidad tras expirar (aunque no se haya purgado), rechazo de UPDATE in-place, `Reap` que solo borra lo realmente vencido y respeta el aislamiento por organización.

## Fase 8 — comparación canaria y reporte final

`internal/rag/postgres/canary_test.go` corre lexical vs. gemini-híbrido vs. bge-m3-híbrido sobre el mismo corpus (7 documentos, incluidos 4 distractores para que "top-3" sea una comparación real, no trivial) y la misma query, contra Postgres real, con vectores sintéticos deterministas (nunca embeddings reales — no hay clave de Gemini ni sidecar BGE-M3 corriendo en este entorno, declarado explícitamente en el test y en el handoff). Resultado real: lexical recall@3=0.50 (no encuentra el documento semántico sin overlap léxico, la limitación que motivó R29), ambos modos híbridos recall@3=1.00, y el documento numéricamente parecido pero incorrecto ("error 2000") nunca aparece en el top-3 en ningún modo — el gate duro "positive 20-vs-2000 confusion" se verifica explícitamente dentro del propio test.

El handoff obligatorio completo (commits, migraciones, comandos corridos, hardware medido, estado de BGE-M3, costo de Gemini, tabla de comparación, resultado de los 14 proyectos, gates duros pasados/fallados, bloqueadores, riesgos pendientes, errores encontrados y corregidos, confirmación de que producción nunca se tocó) vive en `docs/implementation/branch-30-canary-evaluation-bge-m3/HANDOFF.md`. **R30 no se declara completa en el sentido pleno** — ver ese documento para el detalle exacto de qué falta y por qué.
