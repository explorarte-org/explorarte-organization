...
## R30.1 — cierre correctivo (revisión del owner, no autodetectado)

Después de que la Fase 8 quedara commiteada, el owner revisó la rama y encontró 6 hallazgos bloqueantes reales — ninguno de los 6 fue detectado por esta sesión antes de que el owner los señalara. Se verificó cada uno contra el código real (no se asumió que la crítica era correcta sin leer el código) antes de corregir, y los 6 quedaron cerrados con un commit por hallazgo, mismo patrón de verificación completa (`gofmt`, `go vet`, `go build`, `go test ./... -count=1`, `go test -tags=integration ./... -count=1 -p 1`, `make verify`) antes de cada uno:

| Commit | Hallazgo | Qué corrige |
|---|---|---|
| `78214e3` | 1 y 5 | `hybrid_query.go`/`search.go` filtraban el canal vectorial solo por dimensión — un chunk/entrada re-embebido bajo otra revisión podía sumar varios votos RRF o compararse contra un espacio vectorial incompatible que solo compartía dimensión. Se agrega `rag.EmbeddingIdentity`/`memory.EmbeddingIdentity` (model_id+version para Gemini, o revision+artifact_sha256+tokenizer_revision+normalization+pooling para BGE-M3) y la query exige coincidencia exacta. De paso, BGE-M3 local deja de pasar por Pricing/Wallet/Budgets (`LocalComputeOnly`) en vez de sembrar una wallet ficticia de USD 0. |
| `ef4d0db` | 2 | `Reindex` solo generaba chunks, nunca los embebía; `InsertBGEM3ChunkEmbedding` no tenía caller productivo. Se agrega `Manager.BackfillEmbeddings` (rag y memory), reanudable/idempotente, con `orgctl rag backfill-embeddings`/`orgctl memory backfill-embeddings`. |
| `22b1d36` | 3 | `evaluation run` devolvía éxito y marcaba el run completado sin importar cuántas fixtures se hubieran omitido — 4/14 podía leerse como aprobación total. Ahora distingue el hueco honesto (fixtures `StatusPending` de fases futuras) de una cobertura realmente incompleta, y solo marca el run completado cuando se ejecutó todo lo que el catálogo declara `runner_ready`. `compare` rechaza runs incompletos o con conjuntos de fixtures distintos. |
| `41bf047` | 4 | El fixture de página hostil afirmaba "nunca se usa como instrucción" verificando solo `Evidence.Validate()` (tautológico) y "nunca se promueve a RAG/Memory" con un booleano fijo en `true`. Ahora renderiza a un `contextengine.SourceRecord` real (`SourceWebEvidence`, nuevo) y atraviesa el `DeterministicAssembler`/`PortableRenderer` reales, incluyendo una fuente adversarialmente reclasificada para probar que el gate rechaza de verdad. La ausencia de promoción pasa de booleano fijo a `scripts/check-webevidence-fitness.sh` (nuevo, en `make verify`). |
| `3d1690b` | 6 | El adapter BGE-M3 validaba `model_revision`/dimensión en `Embed` pero nunca el hash del artefacto ni el `prompt_template_version` recibido; `Healthy()` sí verificaba el hash pero ningún caller productivo lo invocaba. `embedWireResponse` gana `artifact_sha256`/`prompt_template_version`, `Embed` los verifica, y `openBGEM3SemanticSearch` llama `Healthy()` obligatoriamente al arrancar (falla el `Open` si el sidecar no está listo o tiene otra identidad). |

Los 6 commits están sobre `main`, sin pushear, mismo patrón que el resto de la rama. Esto no cambia la declaración de "no completa" de arriba — BGE-M3 real sigue sin instalarse ni medirse, y los 10 fixtures sin runner siguen pendientes. Lo que cambia es que los defectos reales que el owner encontró en lo que sí se había construido quedaron cerrados, verificados, y documentados — no se declaró "completo" ocultando que un revisor externo, no esta sesión, los encontró.
