...
- `POST /v1/embed` — cuerpo `{"model_revision","prompt_template_version","idempotency_key","items":[{"key","text","task","input_hash"}]}`, respuesta `{"model_revision","artifact_sha256","prompt_template_version","dimension","results":[{"key","vector"}],"text_count","cpu_time_ms"}`. **R30.1-6:** `artifact_sha256` y `prompt_template_version` en la respuesta son obligatorios — el adapter Go rechaza (`ErrModelIdentityDrift`) cualquier discrepancia contra `ArtifactSHA256`/el `prompt_template_version` de la request, exactamente como ya hacía con `model_revision`/`dimension`. Un sidecar que no eche estos dos campos de vuelta hace que todo `Embed` falle, no que se ignore la verificación.
- `GET /v1/health` — respuesta `{"status","model_revision","artifact_sha256","dimension","queue_depth","peak_rss_bytes","cpu_time_ms","processed_count"}`. Endpoint separado de `/v1/embed` a propósito — el healthcheck nunca debe competir por el mismo slot de concurrencia acotada que un embed real. **R30.1-6:** este endpoint ahora se llama obligatoriamente al arrancar el perfil BGE-M3 (`internal/rag/bootstrap`/`internal/memory/bootstrap`, función `openBGEM3SemanticSearch`) — un sidecar caído o con identidad distinta hace fallar el arranque del comando `orgctl` que active ese perfil, en vez de descubrirse recién en la primera consulta real.
