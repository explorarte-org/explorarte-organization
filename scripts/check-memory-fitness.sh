...
require 'data_class IN \(' migrations/000015_create_organizational_memory.up.sql
...
# stores or depend on future vector/RAG packages to decide whether memory is
# publishable.
if grep -R -E 'cell\.read_clinical_data|internal/cell[^w]|clinical_records|patient_records' internal/memory --include='*.go'; then
...
# Approved memory must never become a capability source.
...
# CLI and production wiring must be present.
...
# Compile-facing unit suites are the strongest cheap regression signal here.
