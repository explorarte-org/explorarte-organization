...
MODE="${1:-all}"
case "$MODE" in
  all|tasks|staging|authorization|context|memory|skillregistry|rag|model|egress|dispatch|identity|worker|decision|trace|improvement|completion|composition|mission|shadow|messaging|executive|orgctl) ;;
  *) echo "usage: $0 [all|tasks|staging|authorization|context|memory|skillregistry|rag|model|egress|dispatch|identity|worker|decision|trace|improvement|completion|composition|mission|shadow|messaging|executive|orgctl]" >&2; exit 2 ;;
esac
