...
MODE="${1:-all}"
case "$MODE" in
  all|tasks|staging|authorization|context|memory|skillregistry|rag|model|egress|dispatch|identity|worker|decision|trace|improvement|completion|shadow|messaging|executive) ;;
  *) echo "usage: $0 [all|tasks|staging|authorization|context|memory|skillregistry|rag|model|egress|dispatch|identity|worker|decision|trace|improvement|completion|shadow|messaging|executive]" >&2; exit 2 ;;
esac
