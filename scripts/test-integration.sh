...
MODE="${1:-all}"
case "$MODE" in
  all|tasks|staging|authorization|context|model|egress|dispatch|identity) ;;
  *) echo "usage: $0 [all|tasks|staging|authorization|context|model|egress|dispatch|identity]" >&2; exit 2 ;;
esac
